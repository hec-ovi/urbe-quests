...
const [recordingArg, profile, atlasWorldArg, typesArg, outputArg] = process.argv.slice(2);
if ([recordingArg, profile, atlasWorldArg, typesArg, outputArg].some((arg) => arg === undefined)) {
  throw new Error('usage: materialize.ts <recording.json> <profile> <atlas-or-named-world.json> <npc-types.json> <questlines.json output>');
}
