...
`npm run sample -- "<prompt>" <name> [<named world json> <npc types json>]` ([samples/run-local.ts](samples/run-local.ts), client in [samples/OpenAICompatibleClient.ts](samples/OpenAICompatibleClient.ts)) streams text and indexed tool calls from an OpenAI-compatible Chat Completions endpoint (`LLM_BASE_URL`, `LLM_MODEL`, `LLM_API_KEY`) and writes `samples/<name>/` as each stage lands: `script.md`, `situations.md`, `main.plan.md`, `main.questline.json`, `side-<id>.plan.md`, `side-<id>.questline.json`, `questlines.json`, then `meta.json` (prompt, model, world). Progress goes to stderr with elapsed seconds, one line per text call, build round and finished questline.
