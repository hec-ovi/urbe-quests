...
`npm run materialize -- <recording> <profile> <atlas-or-named-world> <npc-types> <questlines-output> [<handoff-input>]` replays semantic parcel, district and fallback role-type bindings against a concrete city. `npm run bundle -- <sample-dir> [<questlines-output>] [<handoff-input>]` packages a completed sample. Both write the stable [../handoff/CONTRACT.md](../handoff/CONTRACT.md) file set beside the questlines: `objectives.json`, `investigations.json`, `mission-assets.json`, `mission-item-bindings.json`, and `quest-bundle.json`, plus `questlines.meta.json` for materialize. An omitted handoff input means empty investigation and mission catalogs; an authored investigation step without a matching request fails closed. Same inputs produce byte-identical outputs.
