...
- creation/: the creation workflow: script, main translation, situations and side translations from one prompt with one port per stage; progress events per stage and build round. Sample harness (a live endpoint, or a recorded run replayed with no model) and output. Depends on story, builder, world, ports.
