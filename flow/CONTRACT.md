...
- `opening` and `reply` are NPC lines and may carry inline emotion cues, the closed set `CUES` in [cues.ts](cues.ts): `[laugh] [sigh] [whisper] [angry] [gasp] [cry]`, placed where the sound or manner happens. An NPC line needs words besides its cues and holds no other bracketed tag; the player's `text` holds none. Hosts show a line through `stripCues(text)` and hand the raw line to a voice.
