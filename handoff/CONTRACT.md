...
- `stagingProblems(staging)` names what is wrong inside one staging: repeated element ids, an actor with both or neither of `roleId` and `gender`, a quest character in a living pose, a neighbour not listed before its element, a mission asset without the item it shows or a mark with one, a clue shown twice or on no element, a scene both cleared by a step and lasting, and one that appears and clears as the same step is done.
- `stagingConflicts(definition, staging)` names what is wrong against the questline's steps as they stand, all of them or those added so far: a place step that happens in no building, a scene with clues that stands anywhere but at one of its clue steps, and a prop showing an item a pickup step already stands in the city.
- `scopedScenes(definition, stagings)` is a questline and its stagings as a build ships them: each `sceneId` becomes `<questId>.<sceneId>` (`sceneSpecId`) on its staging and on the investigation steps naming it, so questlines built apart never share a scene or an investigation id.
- `stagedScenery(definition, stagings, itemAssets?)` returns `{ scenery, investigations, assets }` under the ids the stagings carry. Each staging becomes one scene spec in the building its `atStepId` step happens in (where it meets its people, goes to or ends at, or where its item lies), on the ground floor, the one floor every building has: a questline does not know how tall its buildings are. A room or story slot without `roomKinds` may be any room. A scene with clues stands at one of its clue steps, so when casting moves that step the scene and its investigation move with it. The scene stands once `stagedBy` is active (`stagedWhen: active`) or done, and a quest character in it only once the simulation holds them dead (`roleDead`). It clears after `clearedBy`, never when `lasting`, else when the questline ends. Seeds and appearance seeds hash the questline, scene and actor ids. The investigation steps naming its `sceneId` become one 1.2 investigation under the same id, and the scene stands before each of them is done (staged by an earlier step, or by the clue step while it is active) and clears no sooner, since a clue is found only while its scene stands: the step's evidence item gives the evidence label and description, each clue is inspected on the element `evidence` names, and a clue on a mission asset is portable and read before it is taken. A mission-asset prop names the asset `itemAssets` binds its item to, else the item's own (`questItemAssetId(questId, itemId)`, the id its pickup would have), so every copy of an item looks the same. `assets` lists each item a scene shows with that asset id; the caller requests and binds it.
- `auditStagings(definition, stagings, scenery)` compiles one questline's stagings and runs the scenery, investigation and host capability audits over them, as the builder does at finish and after pinning.
...
- `scenery`: scene specs unchanged after validation. Scene and investigation ids are unique across the bundle.
...
- A scene names only its own questline's steps, flags, roles and buildings. A quest character stands in it only as a corpse, and its `activeWhen` requires the step that kills them or `roleDead`. A scene and its 1.2 investigation name each other. A staged scene ships under `<questId>.<sceneId>`, and so does the investigation over it.
